...
export class UserserviceProvider {
  ...
  public uid:string;
  private usersDocument: AngularFirestoreDocument<any>;
  ...
  constructor(private storage: AngularFireStorage,public http: HttpClient,private firebaseauth:AngularFireAuth,private afs: AngularFirestore) { 
    ...
    this.sgEvents= this.afs.collection<any>("events", ref => ref.where('type', '==', 'SG Events')).valueChanges()
    this.ritEvents= this.afs.collection<any>("events", ref => ref.where('type', '==', 'RIT Events')).valueChanges();
    this.comHEvents= this.afs.collection<any>("events", ref => ref.where('type', '==', 'Common Hour Events')).valueChanges();
    this.otherEvents= this.afs.collection<any>("events", ref => ref.where('type', '==', 'Other Events')).valueChanges();
    this.sugEvents= this.afs.collection<any>("events", ref => ref.where('type', '==', 'Suggested Events')).valueChanges();
  ...
  signUpUserService(account:User, password){
        ...
        return this.fireAuth.auth.createUserWithEmailAndPassword(account['email'], password).then((newUser) => {
          ...
          this.fireAuth.auth.signInWithEmailAndPassword(account['email'], password).then((updateProfile) =>{
            ...
            const ref = this.storage.ref('profilePics/'+account['fullName']+'/'+cUser.uid);
            var file =this.getprofilePic()
  ...
  addGroup(header,groupName,description,profilePic,type?,location?,deal?){
    switch (header) {
      ...
      return this.athleticsCollection.add(athletics);
      case "RIT Clubs":
      let club={
        about: description,
      ...
      return this.clubsCollection.add(club);
      case "RIT Deals":
        let deals={
          about: description,
          image: profilePic,
        ...
        return this.dealsCollection.add(deals);
      default:
        break;
    }
  }
  ...
  getUserProfile(uid){
    this.usersDocument =this.afs.doc<any>('users/'+uid)
     return this.usersDocument.valueChanges();
  }
